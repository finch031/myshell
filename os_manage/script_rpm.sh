...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='rabbitmq_erlang'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/rabbitmq_erlang.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='rabbitmq_erlang'
  ...
  echo "Generating yum cache for rabbitmq_erlang..."
  yum -q makecache -y --disablerepo='*' --enablerepo='rabbitmq_erlang'

  echo "Generating yum cache for rabbitmq_erlang-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='rabbitmq_erlang-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh rabbitmq_erlang
...
main ()
{
  ...
  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/rabbitmq_erlang.repo
  else
    yum_repo_path=/etc/yum.repos.d/rabbitmq_erlang.repo
  fi
